...
from save_to_mysql import save_shop_rental_item
import time
import platform
import requests


def spider_anjuke_main():
    name_key = [
                   {
                       "name": "wujin",
                       "start": 1,
    ...
    for one_name in name_key2:
        ...
        for num in range(start, max_page):
            ...
            web_url = "https://cz.sydc.anjuke.com/sp-zu/" + name + "-p" + str(
                    ...
                    if item_list is not None:
                        for item in item_list:
                            house = {'title': '', 'address': '', 'price': '', 'price_short': '', 'price_str': '',
                                     'price_unit': '',
                                     'price_daily': '', 'area': '', 'street': '', 'region': ''}
                            ...
                            if item_price is not None:
                                price_div = item_price.select_one('div[class="price-monthly"]')
                                if price_div is not None:
                                    span_list = price_div.select('span')
                                    if span_list is not None and len(span_list) > 0:
                                        price = span_list[0].get_text()
                                        price_unit = span_list[1].get_text()
                                ...
                                price_daily_div = item_price.select_one('div[class="price-daily"]')
                                if price_daily_div is not None:
                                    span_list = price_daily_div.select('span')
                                    if span_list is not None and len(span_list) > 0:
                                        price_daily = span_list[0].get_text()
                                        house['price_daily'] = price_daily
                            ...
                            print(house)
                            save_shop_rental_item(house)
                            print("------------------------------")
            time.sleep(10)


if __name__ == '__main__':
    print("spider of anjuku starts ")
    spider_anjuke_main()
